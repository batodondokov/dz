...
image = np.load("D:\images\wires6.npy.txt")
labeled = label(image)
for i in range(1,labeled.max()+1):
    ...
    if erased_labeled.max() in [2,3,4]:
        print("Провод номер ", i , "разделен на ", erased_labeled.max(), "части")
    elif erased_labeled.max() == 1:
        print("Провод номер ", i , "не разделен ")
    elif erased_labeled.max() == 0:
        print("Провод номер ", i , "не является проводом, так как его толщина не 3 пиксела")
    else:
        print("Провод номер ", i , "разделен на ", erased_labeled.max(), "частей")
plt.imshow(labeled)
plt.show()